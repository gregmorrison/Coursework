...
int main() {
	...
	int iterMax = 10*N*N;            // Set max to avoid excessively long runs
	double changeDesired = 1e-5;  // Stop when the change is given fraction
	Matrix change(iterMax); // Record fractional change at each iteration
	int iter, nIter;        // Iterations counters
	double norm=0;
	
	int chargeloc=(int((double(N)/4.0)));
